...
interface GreenRoomPanelProps {
  showId: string
  open: boolean
  onClose: () => void
}

// Permissie-definitie met label + beschrijving
interface PermissionDef {
  key: keyof GreenRoomPermissions
  label: string
  description: string
}
...
const PERMISSION_DEFS: PermissionDef[] = [
  {
    key: 'view_all_cues',
    label: 'Volledig programma zien',
    description: 'De gast ziet alle cues in het programma. Zet uit om alleen hun eigen cues te tonen.',
  },
  {
    key: 'edit_cues',
    label: 'Cuenaam en locatie aanpassen',
    description: 'De gast mag de naam en locatie van cues zelf wijzigen. Zet uit voor alleen-lezen.',
  },
  {
    key: 'view_tech_notes',
    label: 'Technische notities zien',
    description: 'Toont de technische notities (tech notes) bij elke cue. Standaard verborgen voor gasten.',
  },
  {
    key: 'view_countdown',
    label: 'Live countdown zien',
    description: 'Toont de afteltimer van de lopende cue. Zet uit als de gast geen tijdsdruk hoeft te voelen.',
  },
]
  ...
  return (
    ...
    >
      ...
      <span className={[
        ...
        {value && <Check className="h-2.5 w-2.5" />}
      </span>
...
export function GreenRoomPanel({ showId, open, onClose }: GreenRoomPanelProps) {
  ...
  const [permissions, setPermissions] = useState<GreenRoomPermissions>(DEFAULT_PERMISSIONS)
  ...
  function openEdit(m: CastMember) {
    ...
    // Laad opgeslagen permissies of gebruik standaardwaarden
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const saved = (m as any).permissions
  ...
  function setPermission(key: keyof GreenRoomPermissions, value: boolean) {
  ...
  async function saveMember() {
    ...
    try {
      ...
      } else if (msg.includes('column') && msg.includes('permissions')) {
        setSaveError('De permissions kolom bestaat nog niet. Voer migratie 026 uit in Supabase.')
      } else {
        setSaveError(msg)
      }
    } finally {
  ...
  // Magic link → Green Room login pagina
  function portalUrl(token: string) {
    return `${window.location.origin}/green-room?magic=${token}`
  }
  ...
  function sendInviteEmail(member: CastMember, token: string) {
    ...
    const subject = encodeURIComponent(`Uitnodiging Green Room – ${member.name}`)
    const body = encodeURIComponent(
      `Hallo ${member.name},\n\n` +
      `Je bent uitgenodigd voor de Green Room van dit evenement.\n\n` +
      (member.role ? `Rol: ${member.role}\n\n` : '') +
      `Je kunt het programma bekijken via onderstaande link:\n${loginUrl}\n\n` +
      `Of ga naar /green-room en voer je PIN in:\n\nPIN: ${member.pin}\n\n` +
  ...
  // Samenvatting van actieve permissies voor een member
  function permissionSummary(member: CastMember): string {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const p: GreenRoomPermissions = { ...DEFAULT_PERMISSIONS, ...((member as any).permissions ?? {}) }
    const active: string[] = []
    if (p.view_all_cues)   active.push('Volledig programma')
    if (p.edit_cues)       active.push('Bewerken')
    if (p.view_tech_notes) active.push('Tech notities')
    if (p.view_countdown)  active.push('Countdown')
    return active.length > 0 ? active.join(' · ') : 'Alleen-lezen'
  }
    ...
    <>
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-6 pt-4 space-y-4">
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <Button size="sm" onClick={openAdd} className="gap-1.5 bg-emerald-600 hover:bg-emerald-500 text-black font-bold">
                <UserPlus className="h-4 w-4" /> Gast toevoegen
              </Button>
            </div>
              ...
              <div className="bg-emerald-500/10 border border-emerald-500/25 rounded-xl px-4 py-3 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <code className="text-xs text-emerald-400 flex-1 truncate font-mono">
                    /green-room?magic={quickLink.token.slice(0, 16)}…
                  </code>
                  <Button
            ...
            {loading ? (
              ...
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-sm">Nog geen gasten. Voeg er één toe.</p>
              </div>
            ) : (
                    ...
                    <div key={member.id} className="border border-border/50 rounded-xl overflow-hidden">
                      ...
                      <div className="flex items-center gap-3 px-4 py-3 bg-muted/20">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {/* Permissie-overzicht */}
                          <p className="text-[10px] text-emerald-400/50 mt-0.5 flex items-center gap-1">
                      ...
                      <div className="px-4 py-3 border-t border-border/30">
                        ...
                        {member.pin ? (
                          <div className="mt-1.5 flex items-center gap-2">
                            ...
                            <span className="text-xs text-muted-foreground/40">→ /green-room</span>
                          </div>
                        ) : (
                            ...
                            <div key={link.id} className="space-y-1.5">
                              <div className="flex items-center gap-2 bg-muted/30 rounded-lg px-3 py-2 border border-border/30">
                                <code className="text-[11px] text-emerald-400/80 flex-1 truncate font-mono">
                                  /green-room?magic={link.token.slice(0, 12)}…
                                </code>
      ...
      <Dialog open={!!qrToken} onOpenChange={(o) => { if (!o) setQrToken(null) }}>
        <DialogContent className="max-w-xs text-center">
          <DialogHeader>
            <DialogTitle className="text-center">QR-code Green Room</DialogTitle>
          </DialogHeader>
          {qrToken && (
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="max-w-sm max-h-[90vh] flex flex-col overflow-hidden p-0 gap-0">
          <DialogHeader className="shrink-0 px-6 pt-6 pb-4 border-b border-border/30">
            <DialogTitle>{editTarget ? 'Gast bewerken' : 'Gast toevoegen'}</DialogTitle>
          </DialogHeader>
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <Label className="text-sm font-semibold">Toegang &amp; rechten</Label>
              </div>
              <p className="text-xs text-muted-foreground/60">
                Bepaal wat deze gast mag zien en doen in de Green Room.
              </p>
              <div className="space-y-2 pt-1">
                {PERMISSION_DEFS.map(def => (
                  <PermissionToggle
                    ...
                    onChange={(v) => setPermission(def.key, v)}
                  />
                ))}
              </div>
            </div>
          ...
          <DialogFooter className="shrink-0 px-6 pb-6 pt-2 border-t border-border/30">
