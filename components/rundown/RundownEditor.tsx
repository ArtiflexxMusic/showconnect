...
export function RundownEditor({ rundown: initialRundown, show, initialCues, userId, allRundowns = [], maxCues = null, maxRundowns = null }: RundownEditorProps) {
  ...
  const addCue = useCallback(async (input: CreateCueInput) => {
    ...
    const tempCue: Cue = {
      ...
      title: input.title, type: input.type, secondary_types: input.secondary_types ?? [],
    ...
    const { data, error } = await supabase.from('cues').insert({
      ...
      title:            input.title,
      type:             input.type,
      secondary_types:  input.secondary_types ?? [],
      duration_seconds: input.duration_seconds,
  ...
  const applyTemplate = useCallback(async (templateCues: TemplateCue[], audio: TemplateAudioPayload | null) => {
    ...
    const rows = templateCues.map((tc, i) => ({
      ...
      title:            tc.title,
      type:             tc.type,
      secondary_types:  tc.secondary_types ?? [],
  ...
  const duplicateCue = useCallback(async (cue: Cue) => {
    ...
    await supabase.from('cues').insert({
      ...
      type:             cue.type,
      secondary_types:  cue.secondary_types ?? [],
